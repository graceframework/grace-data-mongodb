...
class GeoJSONTypePersistenceSpec extends GormDatastoreSpec {
    ...
    void 'Test persist abstract shape type'() {
        given: 'A data model based on https://blog.codecentric.de/en/2013/03/mongodb-geospatial-indexing-search-geojson-point-linestring-po.'
        createGeoDataModel()
    ...
    void 'Test Polygons persist'() {
        when:
        Polygon p = Polygon.valueOf([
                Point.valueOf(0, 0),
                Point.valueOf(3, 6),
                Point.valueOf(6, 1),
                Point.valueOf(0, 0)
        ...
        p = Polygon.valueOf( // multi rings with number array as points
                [
                        [Point.valueOf(0, 0), Point.valueOf(3, 6), Point.valueOf(6, 1), Point.valueOf(0,
                                0)], // exterior ring
                        [Point.valueOf(2, 2), Point.valueOf(3, 3), Point.valueOf(4, 2), Point.valueOf(2,
                                2)]  // interior ring
                ]
        )
        l = new Loc(shape: p).save(flush: true, validate: false)
    ...
    void 'Test multi-ring Polygons'() {
        ...
        then:
        Loc.findByShapeGeoIntersects(LineString.valueOf(
                [[-10, 10], [30, 10]])).id == exampleMultiRingedPoly.id // test accross the whole poly
        Loc.findByShapeGeoIntersects(
                LineString.valueOf([[-10, 10], [10, 10]])).id == exampleMultiRingedPoly.id // test over the side
