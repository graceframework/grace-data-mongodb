...
class ReadManyObjectsSpec extends GormDatastoreSpec {
    ...
    void "Test that reading thousands of objects doesn't run out of memory"() {
        ...
        then: 'Check that it does not take too long'
        took < 30000
    }

    void 'Test that reading thousands of objects does not run out of memory native query'() {
        given: 'A lot of test data'
        createData()
    ...
    void createData() {
        ...
        100000.times {
            ProfileDoc.collection.insertOne(
                    new Document(n1: "Plane $it".toString(), n2: it, n3: it.toLong(), date: new Date()))
        }
    }
