...
class EmbeddedWithinEmbeddedAssociationSpec extends GormDatastoreSpec {
    ...
    void 'Test that nested embedded associations can be persisted'() {
        ...
        def vehicle1 = new Vehicle(type: 'car', owners: [customer],
                parts: [new Part(type: 'wheel', maker: maker)]).save(failOnError: true, flush: true)
        def vehicle2 = new Vehicle(type: 'truck', owners: [customer],
                parts: [new Part(type: 'headlight', maker: maker)]).save(failOnError: true, flush: true)
        customer.vehicles = [vehicle1, vehicle2]
        customer.save(failOnError: true, flush: true)
        def vehicle3 = new Vehicle(type: 'bus', owners: [customer],
                parts: [new Part(type: 'plate', maker: maker, components: [new Component(type: 'scribble')])]).save(
