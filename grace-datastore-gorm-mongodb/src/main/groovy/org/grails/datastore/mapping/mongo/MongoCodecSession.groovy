...
class MongoCodecSession extends AbstractMongoSession {
    ...
    protected Map<Class, MongoCodecEntityPersister> mongoCodecEntityPersisterMap =
        new ConcurrentHashMap<Class, MongoCodecEntityPersister>().withDefault { Class type ->
    ...
    MongoCodecSession(MongoDatastore datastore, MappingContext mappingContext,
    ...
    void flush(WriteConcern writeConcern) {
        ...
        try {
            ...
            for (PersistentEntity persistentEntity in pendingDeletes.keySet()) {
                ...
                if (deletes) {
                    ...
                    if (nativeKeys.size() == 1) {
                        entityWrites << new DeleteOneModel<Document>(
                                new Document(MongoEntityPersister.MONGO_ID_FIELD, nativeKeys.get(0)))
                    }
                    else {
                        entityWrites << new DeleteManyModel<Document>(new Document(MongoEntityPersister.MONGO_ID_FIELD,
                                new Document(BsonQuery.IN_OPERATOR, nativeKeys)))
                    }
                }
            }
            ...
            for (PersistentEntity persistentEntity : writeModels.keySet()) {
                ...
                if (wc == null) {
                    org.grails.datastore.mapping.mongo.config.MongoCollection mapping = (org.grails.datastore.
                            mapping.mongo.config.MongoCollection) persistentEntity.mapping.mappedForm
                ...
                if (writes) {
                    final BulkWriteResult bulkWriteResult = collection.bulkWrite(writes)
    ...
    protected List<WriteModel<?>> getWriteModelsForEntity(PersistentEntity persistentEntity,
            Map<PersistentEntity, List<WriteModel<?>>> writeModels) {
    ...
    long updateAll(QueryableCriteria criteria, Map<String, Object> properties) {
        ...
        final UpdateResult updateResult = collection.updateMany(nativeQuery,
                new Document(MONGO_SET_OPERATOR, properties), updateOptions)
