...
class PersistentEntityCodec extends BsonPersistentEntityCodec {
            ...
            for (association in entity.associations) {
                if (association instanceof OneToMany) {
                    ...
                    if (((ToOne) association).isForeignKeyInChild()) {
                        ...
                        query.eq(association.inverseSide.name, access.identifier).projections().id()
                        ...
                        access.setPropertyNoConversion(
                                association.name,
                                lazy ? mongoSession.proxy(associatedClass, (Serializable) id)
                                        : mongoSession.retrieve(associatedClass, (Serializable) id)
                        )
                    }
                }
            }
     ...
     */
    @Override
    Document encodeUpdate(Object value, EntityAccess access = createEntityAccess(value),
            EncoderContext encoderContext = DEFAULT_ENCODER_CONTEXT, boolean embedded = false) {
        ...
        if (value instanceof DirtyCheckable) {
                ...
                if (prop != null) {
                    ...
                    if (v != null) {
                        if (prop instanceof Embedded) {
                            ...
                            PropertyEncoder<? extends PersistentProperty> propertyEncoder = getPropertyEncoder(
                                    (Class<? extends PersistentProperty>) propKind)
            ...
            for (association in entity.associations) {
                if (processed.contains(association.name)) {
                    continue
                }
                if (association instanceof OneToMany) {
            ...
            if (hasSets && isVersioned) {
                ...
                getPropertyEncoder((Class<? extends PersistentProperty>) propKind)?.encode(writer, version, v, access,
    ...
    protected void encodeEmbeddedCollectionUpdate(EntityAccess parentAccess, BsonDocument sets, Document unsets,
            Association association, Object v) {
        if (v instanceof Collection) {
            if ((v instanceof DirtyCheckableCollection) && !((DirtyCheckableCollection) v).hasChangedSize()) {
                ...
                // if this is not a dirty checkable collection or the collection has changed size then a whole new
                // collection has been
                    ...
                    if (rootClass != cls) {
                        ...
                        if (entity == null) {
                            throw new DatastoreException(
                                    "Value [$o] is not a valid type for association [$association]")
                        }
    ...
    static class OneToManyDecoder implements PropertyDecoder<Association> {
        ...
        @Override
        void decode(BsonReader reader, Association property, EntityAccess entityAccess, DecoderContext decoderContext,
    ...
    static class OneToManyEncoder implements PropertyEncoder<Association> {
        ...
        @Override
        void encode(BsonWriter writer, Association property, Object value, EntityAccess parentAccess,
            ...
            if (shouldEncodeIds) {
                ...
                if (value instanceof Collection) {
                    ...
                    if (updateCollection) {
                        ...
                        Collection identifiers = (Collection) mongoSession.getAttribute(parentAccess.entity,
                                "${property}.ids")
                        if (identifiers == null) {
    ...
    static class ToOneEncoder implements PropertyEncoder<ToOne> {
        ...
        @Override
        void encode(BsonWriter writer, ToOne property, Object value, EntityAccess parentAccess,
                EncoderContext encoderContext, CodecRegistry codecRegistry) {
                ...
                if (!property.isForeignKeyInChild()) {
                    ...
                    if (associationId != null) {
                        ...
                        if (attr?.isReference()) {
                            ...
                            MongoCodecSession mongoSession = (MongoCodecSession) AbstractDatastore.retrieveSession(
                                    MongoDatastore)
                            ...
                            Codec<Object> identityEncoder = (Codec<Object>) codecRegistry.get(
                                    (Class<? extends Object>) associationId.getClass())
    ...
    static class ToOneDecoder implements PropertyDecoder<ToOne> {
        ...
        @Override
        void decode(BsonReader bsonReader, ToOne property, EntityAccess entityAccess, DecoderContext decoderContext,
                CodecRegistry codecRegistry) {
            MongoCodecSession mongoSession = (MongoCodecSession) AbstractDatastore.retrieveSession(MongoDatastore)
            MongoAttribute attr = (MongoAttribute) property.mapping.mappedForm
