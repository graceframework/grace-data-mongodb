...
You can use MongoDB https://www.mongodb.com/docs/manual/reference/method/cursor.hint/#mongodb-method-cursor.hint[Query Hints] by passing the `hint` argument to any dynamic finder:
...
MongoDB supports the notion of https://www.mongodb.com/docs/manual/core/indexes/index-types/index-compound/[compound keys]. Grace Data MongoDB enables this feature at the mapping level using the `compoundIndex` mapping:
...
In the above example I define an index on an embedded attribtue of the document. In fact what arguments you pass to the `index` method get passed to the underlying MongoDB https://mongodb.github.io/mongo-java-driver/5.0/apidocs/mongodb-driver-legacy/com/mongodb/DBCollection.html#createIndex(com.mongodb.DBObject,com.mongodb.DBObject)[createIndex] method.
