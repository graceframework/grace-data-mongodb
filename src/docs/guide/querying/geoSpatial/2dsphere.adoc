...
MongoDB's https://www.mongodb.com/docs/manual/core/indexes/index-types/geospatial/2dsphere/#std-label-2dsphere-index[2dsphere indexes] support queries that calculate geometries on an earth-like sphere.

Although you can use coordinate pairs in a 2dsphere index, they are considered legacy by the MongoDB documentation and it is recommended you store data using https://datatracker.ietf.org/doc/html/rfc7946#section-3.1.2[GeoJSON Point] types.
...
To support this Grace Data MongoDB features a special type, `grails.mongodb.geo.Point`, that can be used within domain classes to store geospacial data:
...
The `Point` type gets persisted as a https://datatracker.ietf.org/doc/html/rfc7946#section-3.1.2[GeoJSON Point]. A `Point` can be constructed from coordinates represented in longitude and latitude (the inverse of 2d index location coordinates!). Example:
...
In the above example the native query parameters are simply passed to the https://www.mongodb.com/docs/manual/reference/operator/query/near/[$near query]
