Grace Data MongoDB will persist all common known Java types like String, Integer, URL etc., however if you want to persist one of your own classes that is not a domain class you can implement a custom user type.

=== Custom Codecs

Grace Data MongoDB is built ontop of MongoDB's BSON encoding framework. This means it is possible to implement custom https://mongodb.github.io/mongo-java-driver/5.0/bson/codecs/[Codecs] for encoding and decoding values to and from BSON.
...
NOTE: Custom types should go in `src/groovy` not `app/domain`
