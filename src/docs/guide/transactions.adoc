MongoDB doesn't support transactions directly, however Grace Data MongoDB does batch up inserts and updates until the session is flushed. This makes it possible to support some rollback options.
...
Using the lower level API you can of course also take advantage of Mongo's support for https://www.mongodb.com/docs/manual/core/write-operations-atomicity/[Atomic operations].
