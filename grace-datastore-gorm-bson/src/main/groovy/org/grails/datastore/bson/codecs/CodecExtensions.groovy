...
/**
 * Additional {@link Codec} extensions. This class implements {@CodecProvider} and integrates with the
 * {@CodecRegistry} to make it easier to convert objects to and from BSON documents
 *
...
class CodecExtensions implements CodecProvider {
    ...
    private static final Map<Class<? extends BsonValue>, List<Converter>> BSON_VALUE_CONVERTERS =
            new LinkedHashMap<Class<? extends BsonValue>, List<Converter>>()
                    .withDefault { Class<? extends BsonValue> cls -> new ArrayList<Converter>() as List<Converter> }
    ...
    static class MapCodec implements Codec<Map<String, Object>>, CodecRegistryAware {
        ...
        Class<Map<String, Object>> getEncoderClass() {
            Map as Class<Map<String, Object>>
        }

        protected BsonValue readValue(final BsonReader reader, final DecoderContext decoderContext) {
            return codecRegistry.get(BsonValueCodecProvider.getClassForBsonType(reader.getCurrentBsonType())).decode(
