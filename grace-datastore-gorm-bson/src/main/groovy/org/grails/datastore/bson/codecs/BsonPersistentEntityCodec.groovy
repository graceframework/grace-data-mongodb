...
class BsonPersistentEntityCodec implements Codec {
        ...
        while (bsonType != BsonType.END_OF_DOCUMENT) {
            ...
            if (!abortReading) {
                ...
                if (isIdentifierProperty(name)) {
                    getPropertyDecoder(Identity).decode(bsonReader, (Identity) persistentEntity.identity, access,
                            decoderContext, codecRegistry)
                    ...
                    if (property && bsonType != BsonType.NULL) {
                        ...
                        if (CharSequence.isAssignableFrom(property.type) && bsonType == BsonType.STRING) {
                            ...
                            getPropertyDecoder((Class<? extends PersistentProperty>) propKind)?.decode(bsonReader,
                                    property, access, decoderContext, codecRegistry)
                        }
                    }
    ...
    void encode(BsonWriter writer, Object value, EncoderContext encoderContext, boolean includeIdentifier) {
        ...
        if (includeIdentifier) {
            ...
            if (id != null) {
                getPropertyEncoder(Identity).encode writer, (Identity) entity.identity, id, access, encoderContext,
                        codecRegistry
            }
        }
        ...
        for (PersistentProperty prop in entity.persistentProperties) {
            ...
            if (v != null) {
                PropertyEncoder<? extends PersistentProperty> encoder = getPropertyEncoder(
     ...
     * @return A Bson
     */
    Bson encodeUpdate(Object value, EntityAccess access = createEntityAccess(value),
                ...
                if (prop != null) {
                    ...
                    if (v != null) {
                        if (prop instanceof Embedded) {
                            ...
                            encodeUpdate(v, createEntityAccess(((Embedded) prop).associatedEntity, v), encoderContext,
                                    true)
                        }
     ...
     */
    protected void readSchemaless(BsonReader bsonReader, DynamicAttributes dynamicAttributes, String name,
